...
let formula = document.querySelector(".formula").value.replaceAll("√", "math.sqrt");
let formulaF = (x) => eval(formula.replaceAll("x", x));
...
function renderGraph() {
    ...
    for (let i = -499; i <= 499; i += 0.1) {
        ctx.lineTo(i * scale, -formulaF(i) * scale);
    }
    ctx.stroke();
}
...
document.querySelector(".formula").addEventListener("input", (event) => {
    ...
    event.target.value = event.target.value
        ...
        .join("");
    formula = event.target.value.replaceAll("√", "math.sqrt");
    console.log(formula);
